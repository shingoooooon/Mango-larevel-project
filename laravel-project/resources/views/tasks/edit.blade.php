        ...
        <form method="post" action=" {{ route('tasks.update', ['folder' => $task->folder_id, 'task' => $task->id]) }}">
            ...
            <div class="relative z-0 mb-6 w-full group">
                <div class="relative">
                    ...
                    <input datepicker datepicker-autohide name="due_date" type="text" value="{{ old('due_date') ?? $task->due_date }}" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Select date">
                </div>
                @error('due_date')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
